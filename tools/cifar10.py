...
parser.add_argument('-s', '--shift-depth', type=int, default=1000,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.shift_depth > 0:
        model,_ = convert_to_shift(model, args.shift_depth, args.shift_type, convert_all_linear=True, convert_weights=True, use_kernel = False, use_cuda = True, rounding = args.rounding, weight_bits = args.weight_bits, act_integer_bits = activation_integer_bits, act_fraction_bits = activation_fraction_bits)
    elif args.use_kernel and args.shift_depth == 0:
        model = convert_to_unoptimized(model)

    print('Coverted ViT Model:\n{}'.format(model))
    print(model.state_dict)
    if args.distributed:
    ...
    model_summary = None
    # try:
    #     model_summary, model_params_info = torchsummary.summary_string(model, input_size=(3,224,224))
    #     print(model_summary)
    ...
    model_name = '%s/%s%s' % ("vit", shift_label, desc_label)
