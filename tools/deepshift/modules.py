...
class LinearShift(nn.Module):
    def __init__(self, weight,in_features, out_features, bias=True, Use_B = True,check_grad=False, freeze_sign=False, use_kernel=False, use_cuda=True, rounding='deterministic', weight_bits=5, act_integer_bits=16, act_fraction_bits=16):
        ...
        if Use_B:
            self.oriweight = weight
            self.Be = nn.Parameter(tensor_constructor(64, 64),requires_grad=False)
        else:
            self.register_parameter('Be', None)
            self.register_parameter('oriweight', None)
        self.shift = nn.Parameter(tensor_constructor(out_features, in_features))#这里的shift就是Ce,且只有指数，没有符号
    ...
    def reset_parameters(self):
        ...
        if self.bias is not None:
            fan_in, _ = init._calculate_fan_in_and_fan_out(self.shift)# fan_in = in_features
            bound = 1 / math.sqrt(fan_in)
            init.uniform_(self.bias, -bound, bound)

    def forward(self, input):
        self.shift.data = ste.clamp(self.shift.data, *self.shift_range)
        shift_rounded = ste.round(self.shift, rounding=self.rounding)#变为最近的整数
        ...
        ## 加入针对Be的操作
        if self.Be != None:
            if self.oriweight.shape[0]< 128:
                return torch.nn.functional.linear(input, weight_ps, self.bias)
            else:
                As =[]
                Ces = torch.chunk(weight_ps,int(weight_ps.shape[0]/64),0)# 有符号操作
                for element in Ces:
                    A = torch.mm(self.Be.data,element)
                    As.append(A)
                weight_final = torch.cat(As,0)
        else:
            weight_final = weight_ps
        ...
        if self.use_kernel:
            ...
            return torch.nn.functional.linear(input, weight_final, self.bias)
